...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public long insertRow( Date datetime, String description ) {
        long rowId = 0;
        ContentValues val = new ContentValues();
        SQLiteDatabase db = getWritableDatabase();

        if (datetime == null || description == null)
            throw new IllegalArgumentException("insertRow() No null args here!");

        val.put( FORECAST_DATETIME, datetime.getTime() );
        val.put( FORECAST_DESCRIPTION, description );

        rowId = db.insertWithOnConflict( FORECAST_TABLE, null, val, SQLiteDatabase.CONFLICT_REPLACE);

        if ( rowId <= 0 )
            Log.e( DATABASE_NAME, "Row not created: " + datetime.toString() );
    ...
    public List<Forecast> getForecasts() {
        ...
        String[] PROJECTION_D = { FORECAST_DATETIME, FORECAST_DESCRIPTION };
        String SELECTION_W = "";
